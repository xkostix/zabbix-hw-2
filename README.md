...
Заметка: Поскольку в шаблоне `Linux by Zabbix Agent` уже существует ключ, указанный в задании 1, заббикс не дает добавить мой шаблон. Пришлось изменить ключ в моем шаблоне для Item "my_cpu_util %", изменив аргументы:
```
